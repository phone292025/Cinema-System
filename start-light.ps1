...
$LogFile = Join-Path $LogDir "start-light.log"
...
if ($LASTEXITCODE -ne 0) {
  ...
  Write-Host ".\start-light.ps1"
  exit 1
}

$env:COMPOSE_PROGRESS = "quiet"

Write-Host "Starting Cinema without rebuilding. Output is saved to logs/start-light.log"
docker compose up -d --no-build *> $LogFile
if ($LASTEXITCODE -ne 0) {
  Write-Host "Start failed. The Docker images may not exist yet."
  Write-Host "Run .\rebuild-once.ps1 one time, then use .\start-light.ps1 after that."
